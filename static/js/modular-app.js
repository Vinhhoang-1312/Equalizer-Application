...
class AdvancedAudioApp {
    ...
    displayNoiseResults(result) {
        ...
        if (snrImprovementElement && result.snr_improvement) {
            snrImprovementElement.textContent = '+' + result.snr_improvement.toFixed(1);
        }
        
        // Show method used
        const methodElement = document.getElementById('methodUsedNoise');
        if (methodElement) {
            const methodName = result.method === 'autoencoder' ? 'AI/ML Neural Network' : 'Advanced Library';
    ...
    displayNoiseComparisonChart(result) {
        ...
        comparisonImg.src = '/static/results/noise_reduction_comparison.png?' + new Date().getTime(); // Add timestamp to prevent caching
        ...
        if (!comparisonContainer.querySelector('.comparison-title')) {
            ...
            title.innerHTML = '<i class="fas fa-chart-area"></i> Before vs After Comparison';
